...
class SearchSaver
{
    ...
    {
        $data['albums'] = $data['albums']->concat(
            $data['tracks']->pluck('album'),
        );

        // pull artists from albums array
        $data['artists'] = $data['artists']->concat(
            $data['albums']->pluck('artists')->flatten(1),
        );

        // pull albums and artists from tracks array
        $data['artists'] = $data['artists']->concat(
            $data['tracks']->pluck('artists')->flatten(1),
        );
    ...
    private function saveAndGetData($merged, $original)
    {
        ...
        $savedArtists = $savedArtists
            ->filter(function ($savedArtist) use ($original) {
                return $original['artists']->contains(
                    'spotify_id',
        ...
        $savedAlbums = $savedAlbums
            ->filter(function ($savedAlbum) use ($original) {
                return $original['albums']->contains(
                    'spotify_id',
        ...
        $savedTracks = $savedTracks
            ->filter(function ($savedTrack) use ($original) {
                return $original['tracks']->contains(
                    'spotify_id',
